...
-module(classifier_client).
-author('@msgpack-idl').

-include("classifier_types.hrl").
-export([connect/3, connect/4, close/0, close/1]).



-export([set_config/2, get_config/1, train/2, classify/2, save/2, load/2, get_status/1]).  
...
-spec train(mp_string(), list({mp_string(), datum()})) -> non_neg_integer().
train(Name, Data) ->
    gen_msgpack_rpc:call(?MODULE, train, [Name, Data]).

-spec classify(mp_string(), list(datum())) -> list(list(estimate_result())).
classify(Name, Data) ->
    gen_msgpack_rpc:call(?MODULE, classify, [Name, Data]).
